...
(function ($) {
    $.fn.date = function (options, Ycallback, Ncallback) {
        ...
        var initH = parseInt((nowdate.getFullYear() - opts.beginyear));
        //var initI = parseInt(nowdate.getMinutes());
        var initI = parseInt(nowdate.getMonth() + "") + 1;
        ...
        /*www.sucaijiayuan.com*/
            ...
            //绑定事件（默认事件为获取焦点）
            var click = false;
            that.on('touchstart touchmove touchend', function (ev) {//给document添加点击事件代理
                switch (event.type) {
                    case 'touchstart':
                        click = false;
                        break;
                    ...
                    case 'touchend':
                        if (!click) {
                            if (opts.showFuture) {
                                ...
                                opts.endhour = date.getFullYear();
                                opts.endminute = date.getMonth() + 1;
                                opts.endsecond = date.getDate();
                            }
                            if (that.data('start')) {
            ...
            $("#dateconfirm").unbind('click').click(function () {
                ...
                if (datetime) {
                    ...
                    datestr += " " + Hourwrapper.find("ul li:eq(" + indexH + ")").html().substr(0, Hourwrapper.find("ul li:eq(" + indexH + ")").html().length) + "-" +
                        ...
                        Secondwrapper.find("ul li:eq(" + indexS + ")").html().substr(0, Secondwrapper.find("ul li:eq(" + indexS + ")").html().length);
                    indexS = 0;
                }
        ...
        function init_iScroll_datetime() {
            ...
            HourScroll = new iScroll("Hourwrapper", {
                ...
                onScrollEnd: function () {
                    ...
                    if (opts.showFuture) {
                        opts.endhour = checkMonths(showYear);
                        Minutewrapper.find("ul").html(createMONTH_UL());
                        MinuteScroll.refresh();
                        indexI = Math.round((MinuteScroll.y / 96) * (-1) + 1);
                        opts.endminute = checkdays(showYear, indexI);
                        Secondwrapper.find("ul").html(createDAY_UL());
                        SecondScroll.refresh();
                    } else {
                        indexI = Math.round((MinuteScroll.y / 96) * (-1) + 1);
                        opts.endsecond = checkdays(showYear, indexI);
                        Secondwrapper.find("ul").html(createDAY_UL());
                        SecondScroll.refresh();
                    }
                }
            ...
            MinuteScroll = new iScroll("Minutewrapper", {
                snap: "li", vScrollbar: false,
                onScrollEnd: function () {
                    var showYear = Math.round((HourScroll.y / 96) * (-1) + opts.beginyear);
                    if (opts.showFuture) {
                        indexH = Math.round((HourScroll.y / 96) * (-1) + 1);
                        indexI = Math.round((this.y / 96) * (-1) + 1);
                        opts.endday = checkdays(showYear, indexI);
                        Secondwrapper.find("ul").html(createDAY_UL());
                        SecondScroll.refresh();
                    } else {
                        indexI = Math.round((this.y / 96) * (-1) + 1);
        ...
        function CreateDateUI() {
            ...
            var str = '' +
                ...
                '<section>' +
                '<div id="datetitle"><h1 class="language" data-language="chooseTime">请选择日期</h1></div>' +
                ...
                '<div id="startDate" class="language" data-language="Starting_time">开始时间:</div>' +
                '<div id="datescroll">' +
                '<div id="yearwrapper">' +
                '<ul></ul>' +
                '</div>' +
                ...
                '</div>' +
                '</div>' +
                '<div id="finalDate" class="language" data-language="End_Time">结束时间:</div>' +
        ...
        function addTimeStyle() {
            ...
            datePage.css("height", "942px");
            datePage.css("top", "112px");
            var yearwrapper = $('#yearwrapper');
            yearwrapper.css("position", "absolute");
            yearwrapper.css("bottom", "493px");
            yearwrapper.css("top", "171px");
            var monthwrapper = $('#monthwrapper');
            monthwrapper.css("position", "absolute");
            monthwrapper.css("bottom", "493px");
            monthwrapper.css("top", "171px");
            var daywrapper = $('#daywrapper');
            daywrapper.css("position", "absolute");
            daywrapper.css("bottom", "493px");
            daywrapper.css("top", "171px");
            var datemark = $('#datemark');
            datemark.css('top', '290px');
            //第二个
            var timemark = $('#timemark');
            timemark.css('top', '663px');
            var Hourwrapper = $('#Hourwrapper');
            Hourwrapper.css('top', '544px');
            Hourwrapper.css('height', '288px');
            var Minutewrapper = $('#Minutewrapper');
            Minutewrapper.css('top', '544px');
            Minutewrapper.css('height', '288px');
            var Secondwrapper = $('#Secondwrapper');
            Secondwrapper.css('top', '544px');
            Secondwrapper.css('height', '288px');
        }
